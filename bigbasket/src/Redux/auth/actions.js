...
export  const UserLogin_request=(data)=>(dispatch)=>{
    ...
    try{
        return axios.post("https://bigbasket-auth.vercel.app/user/login",data)
...
export  const UserSingin_request=(data)=>(dispatch)=>{
    ...
    try{
       ...
       return  axios.post("https://bigbasket-auth.vercel.app/user/ragister",data).then(res=>dispatch({type:Singin_sucess})).catch(err=>dispatch({type:Singin_error}))
    }
    catch(err){
